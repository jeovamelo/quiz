 ...
 * até 2 controles remotos (slot 1 e slot 2), atribuídos por ordem de
 * chegada do QR Code de pareamento.
 */
...
/**
 * Tenta reivindicar o próximo slot livre (1 ou 2). Se o `deviceToken` já
 * ocupa um slot, devolve esse mesmo slot (re-entrada). Retorna null se
 * ambos os slots já estiverem ocupados por outros aparelhos.
 */
export async function claimRemoteSlot(
  ...
  // 2. Verifica quais slots estão ocupados.
  const { data: rows } = await supabase
    .from("session_remotes")
    .select("slot")
    .eq("session_id", sessionId);
  const taken = new Set((rows ?? []).map((r: any) => r.slot as number));
  for (const slot of [1, 2] as const) {
    ...
    // Caso de corrida (unique violation): tenta o próximo slot.
  }
  return null;
}
